...
static void					Configure_IWDG		( void );
...
static void OneSecThread(const void *argument)
{
	...
	// Config and start IWDT
	Configure_IWDG();

	/* one second circle */
	for( TickType_t startTime = 0, oneSecTickDelayMs = 0, passTime = 0; ;
			passTime = xTaskGetTickCount() - startTime, LL_IWDG_ReloadCounter(IWDG) )
	{
...
static void Configure_IWDG(void)
{
  ...
  LL_IWDG_SetPrescaler(IWDG, LL_IWDG_PRESCALER_32);  	/* (3)  - 1150 Hz*/
  LL_IWDG_SetReloadCounter(IWDG, 0xB60);            	/* (4)  - 2500 ms timeout */
